...
export default function EditDishVariantPage({ params }) {
  ...
  // Resolve params untuk Next.js 15
  useEffect(() => {
    const resolveParams = async () => {
      try {
        ...
        toast.error('Error loading page parameters');
        router.back();
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch variant data dan dishes secara parallel
        const [variantResponse, dishesResponse] = await Promise.all([
          fetch(`/api/dish-variants/${variantId}`),
          fetch('/api/dishes?available=true')
        ]);
        ...
        const formDataFromAPI = {
          dish_id: variant.dish_id?.toString() || '',
          name: variant.name || '',
          type: variant.type || '',
          price_modifier: variant.price_modifier?.toString() || '0',
          is_default: variant.is_default || false,
          is_available: variant.is_available !== false, // Default to true if undefined
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(result.message || 'Varian berhasil diperbarui');
        router.push('/dashboard/dish-variants');
      } else {
        toast.error(result.error || 'Gagal memperbarui varian');
      }
    } catch (error) {
      console.error('Error updating dish variant:', error);
      toast.error('Terjadi kesalahan saat memperbarui varian');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Edit Varian Menu</h1>
          <p className="text-muted-foreground">
            Perbarui informasi varian menu
          </p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Informasi dasar tentang varian menu
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
